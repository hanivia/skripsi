    ...
    def aturan(fungsi_awal):
        def perubahan_halaman(request, *args, **kwargs):
            ...
            if group in yang_diizinkan :
                ...
                return HttpResponse('<h2><center>Anda Tidak Memiliki Hak Akses Ke Halaman Ini</center></h2>')
        return perubahan_halaman
    return aturan
...
def pilihan_login(fungsi_awal):
    def perubahan_halaman(request, *args, **kwargs):
        ...
        if group == 'lembaga' :
            return redirect('user-lembaga')
        
        if group == 'wilayah' :
            return redirect('user-wilayah')

        if group == 'diniyah' :
            return redirect('user-diniyah')
